    ...
    def __init__(
        ...
        if config is None:
            if pretrained:
                ...
                print(f"[日志] HFTextEncoder - 准备调用 AutoModel.from_config，配置来自: {model_name_or_path}")
                print(f"[日志] config.auto_map: {getattr(self.config, 'auto_map', 'None')}")
                ...
                print(f"[日志] AutoModel.from_config 完成，实际创建的模型类型: {type(self.transformer)}")
                print(f"[日志] 模型模块文件: {self.transformer.__class__.__module__}")
                if hasattr(self.transformer.__class__, '__file__'):
                    print(f"[日志] 模型类文件路径: {self.transformer.__class__.__file__}")
                import sys
                for name, module in sys.modules.items():
                    if 'modeling_lora' in name and hasattr(module, '__file__'):
                        print(f"[日志] 找到 modeling_lora 模块: {name} -> {module.__file__}")
            if (
