...
#
# class FlashSelfAttention1(nn.Module):
...
#
# class FlashCrossAttention1(nn.Module):
#     """Implement the scaled dot product attention with softmax.
#     Arguments
#     ---------
#         softmax_scale: The temperature to use for the softmax attention.
...
class MHA(nn.Module):
    ...
    def __init__(
        ...
        if use_alibi:# not here
            assert use_flash_attn, "ALiBi code path requires flash_attn"
            alibi_slopes = torch.tensor(get_alibi_slopes(num_heads), device=device)
        else:
            alibi_slopes = None
        if window_size != (-1, -1):# not here
            assert (
                use_flash_attn
        ...
        linear_cls = nn.Linear if not fused_bias_fc else FusedDense## 前者
        linear_resid_cls = (
            LinearResidual
            if not fused_bias_fc ## here
            else partial(FusedDense, return_residual=True)
        )
        wqkv_cls = linear_cls if not self.return_residual else linear_resid_cls # 后者
        # inner_attn_cls = (
        #     partial(
        #         FlashSelfAttention, alibi_slopes=alibi_slopes, window_size=window_size
        #     )
        #     if use_flash_attn
        #     else SelfAttention   ####后者
        # )
    ...
    def forward(
        ...
        x, #[1 8 1024]
        x_kv=None,#none
        key_padding_mask=None,#[1 8]
        cu_seqlens=None,#none
        max_seqlen=None,#none
        mixer_subset=None,#none
        inference_params=None,#none
        adapter_mask=None,#[0]
        **kwargs,
    ):
        """
        ...
        """
        # if cu_seqlens is not None:# not here
        #     assert max_seqlen is not None
        ...
        # if inference_params is not None:# not here
        #     assert key_padding_mask is None
        ...
        if not self.cross_attn and self.num_heads_kv == self.num_heads:## here
            assert x_kv is None and mixer_subset is None

            if adapter_mask is not None:  # here
                unique_tasks = torch.unique(adapter_mask)#[0]
                ...
                for task_id in unique_tasks:
                    task_indices = (adapter_mask == task_id).nonzero(as_tuple=True)[0]#[0]
                    task_tensor = x[task_indices]# [1 8 1024]same
                    # if not self.return_residual:## not here
                    #     task_qkv = self.Wqkv(task_tensor, task_id=task_id)
                    #else: # here
                    if 1:
                        #task_qkv, _ = self.Wqkv(task_tensor, task_id=task_id, residual=True)##taskqkv=[1 8 3072] taskid=0 change
                        #task_qkv,_=util_weightDirect.linear_Wqkv(task_tensor)
                        task_qkv, _ = self.Wqkv(task_tensor, #task_id=task_id, residual=True
                                                )
                    qkv[task_indices] = task_qkv;#[-0.7495, -0.0796, -0.3332,  0.9270,  0.9582]
                    #print(doneHere)
            else:
            ...
            if (
                ...
                if inference_params is None:# here
                    if not self.checkpointing:## here
                        context = self.inner_attn(qkv, **kwargs)##context[1 8 16 64]
            #         else: # not here
        ...
        inp = rearrange(context, "... h d -> ... (h d)")##[1 8 16 64]->[1 8 1024]
        if adapter_mask is not None:# here
            ...
            for task_id in unique_tasks:
                task_indices = (adapter_mask == task_id).nonzero(as_tuple=True)[0]#[0]
                task_tensor = inp[task_indices]# [1 8 1024]
                #task_out = self.out_proj(task_tensor, task_id=task_id)# [1 8 1024]
                #task_out=util_weightDirect.linear_outproj(task_tensor);print('outproj',task_out[0,0,:5])#[-0.3328,  0.1617,  0.3784, -0.5201,  0.2075]
