...
class XLMRobertaEmbeddings(nn.Module):
    def __init__(
        ...
        #     )
        if self.type_vocab_size > 0:#here
    ...
    def forward(
        ...
        if adapter_mask is not None:# here
            ...
            for task_id in unique_tasks:
                ...
                #task_embeddings = self.word_embeddings(task_input_ids, task_id=task_id)# [1 8 1024]
                #task_embeddings=word_emb(task_input_ids)
        ...
        if self.type_vocab_size > 0:# here
            if token_type_ids is None:# here
            ...
            if adapter_mask is not None:# here
                unique_tasks = torch.unique(adapter_mask)
                for task_id in unique_tasks:
                    #task_token_type_embeddings = self.token_type_embeddings(token_type_ids, task_id=task_id)# token_type_id all 0  | emb[8 1024]
                    #task_token_type_embeddings=tokenTypeId_emb(token_type_ids)#emb[8 1024]
                    task_token_type_embeddings = self.token_type_embeddings(token_type_ids)####227
                    # print('xlmRobertEmb()tokenTyp',LA.matrix_norm(self.token_type_embeddings.weight.data),LA.matrix_norm(task_token_type_embeddings))
                    task_indices = (adapter_mask == task_id).nonzero(as_tuple=True)[0]
