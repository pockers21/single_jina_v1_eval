...
for k,v in W.items():
    ...
    layers_set = {f"layers.{i}." for i in range(1, 24)}
    flag=all(layer not in k for layer in layers_set)
    if flag:
        totalMergeLoraPart_dict[k]=v
    print()
torch.save(totalMergeLoraPart_dict,'/root/autodl-tmp/jina/pytorch_model_mergeLoraPartwrong.bin')
#
