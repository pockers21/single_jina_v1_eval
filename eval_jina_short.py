#!/usr/bin/env python3
"""
JinaCLIP-v2模型召回能力评估脚本
基于MetaCLIP和AltCLIP评测框架，确保与其他模型使用相同的测评参数
使用JinaCLIP-v2模型进行文搜图、图搜文、文搜文三种测试类型
支持5个数据集的全面评估
"""
...
class JinaCLIPWrapper:
    ...
    def _load_model(self):
        """按load_jina.py的方式加载JinaCLIP模型（SentenceTransformer）"""
        print_progress("开始加载JinaCLIP-v2模型 (SentenceTransformer)", self.model_name)

        def _set_max_seq_len(model: SentenceTransformer, max_length: int = 512) -> None:
            try:
                print("max len", getattr(model, "max_seq_length", None))
        ...
        try:
            ...
            _set_max_seq_len(self.model)  # 默认512
            try:
                self.model.half()
            except Exception:
                pass
    ...
    def encode_text_batch(self, texts: List[str], batch_size: int = 32) -> np.ndarray:
        """批量编码文本（使用SentenceTransformer.encode，与load_jina.py一致）"""
    ...
    def encode_image_batch(self, image_paths: List[str], batch_size: int = 32) -> np.ndarray:
        """批量编码图像（使用SentenceTransformer.encode与load_jina.py的风格保持一致）"""
                ...
                if images:
                    with torch.no_grad():
                        ...
                        try:
                            ...
                            # 某些版本无is_image参数，直接尝试encode(images)
                            image_features = self.model.encode(
                                images,
                                batch_size=len(images),
                                convert_to_numpy=True,
...
def evaluate_jinaclip_model(datasets_config: Dict, batch_size: int, results_dir: str) -> Dict:
    """评估JinaCLIP-v2模型"""
    print_progress("开始评估 JinaCLIP-v2 模型", "")
...
def main():
    print_progress("开始JinaCLIP-v2模型召回能力评估", "")
    
    # 配置路径（与MetaCLIP和AltCLIP框架一致）
    datasets_dir = "/root/autodl-tmp/bge_vl/eval/datasets"
    results_dir = "/root/autodl-tmp/bge_vl/datasets/single_jina_eval/short_results"
    
    os.makedirs(results_dir, exist_ok=True)
    
    batch_size = 512  # 与MetaCLIP和AltCLIP框架一致
    
    # 数据集配置（与MetaCLIP和AltCLIP框架完全一致）
    datasets_config = {
        'wukong': {
            'type': 'image_text',
