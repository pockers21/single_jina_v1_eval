...
"""
JinaCLIP-v2长文本评估脚本
基于jinaclip_eval.py，对长评估数据集进行文搜图、图搜文、文搜文三种测试
...
# 模型路径与截断维度与 load_jina.py 对齐
MODEL_PATH = "/root/autodl-tmp/bge_vl/datasets/single_jina_eval/jina_model"
TRUNCATE_DIM = 1024
...
class JinaCLIPWrapper:
    ...
    def __init__(self):
        self.model_name = "JinaCLIP-v2"
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.truncate_dim = TRUNCATE_DIM
        self._load_model()
        
    def _set_max_seq_len(self, max_length: int = 512) -> None:
        try:
            _ = getattr(self.model, 'max_seq_length', None)
    ...
    def _load_model(self):
        """加载JinaCLIP-v2模型"""
        print_progress("开始加载JinaCLIP-v2模型", self.model_name)
        
        try:
            model_dir = MODEL_PATH
            # 与 load_jina.py 对齐：使用 SentenceTransformer，信任远程代码与截断维度
            ...
            print_progress("JinaCLIP-v2模型加载成功", self.model_name)
            
        except Exception as e:
            print_progress(f"JinaCLIP-v2模型加载失败: {str(e)}", self.model_name)
            raise e
    ...
    def encode_text_batch(self, texts: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        for i in tqdm(range(0, len(texts), batch_size), desc=f"[{self.model_name}] 编码文本批次"):
            ...
            try:
                with torch.no_grad():
                    # SentenceTransformer 直接编码文本
                    text_features = self.model.encode(batch_texts)
                    if isinstance(text_features, torch.Tensor):
                        text_features = text_features.cpu().numpy()
                    if text_features.ndim == 1:
                        all_embeddings.append(text_features)
                    else:
                        all_embeddings.extend([feat for feat in text_features])
    ...
    def encode_image_batch(self, image_paths: List[str], batch_size: int = 32) -> np.ndarray:
        ...
        for i in tqdm(range(0, len(image_paths), batch_size), desc=f"[{self.model_name}] 编码图像批次"):
            ...
            try:
                ...
                if images:
                    with torch.no_grad():
                        # SentenceTransformer 直接编码图像(PIL)
                        image_features = self.model.encode(images)
                        if isinstance(image_features, torch.Tensor):
                            image_features = image_features.cpu().numpy()
                        if image_features.ndim == 1:
                            all_embeddings.append(image_features)
                        else:
                            all_embeddings.extend([feat for feat in image_features])
...
class RecallEvaluator:
    ...
    def __init__(self, model: JinaCLIPWrapper):
        self.model = model
...
def evaluate_jinaclip_long_model(datasets_config: Dict, batch_size: int, results_dir: str) -> Dict:
    """评估JinaCLIP-v2长文本模型"""
    print_progress("开始评估 JinaCLIP-v2 长文本模型", "")
    ...
    try:
        ...
        for dataset_idx, (dataset_name, config) in enumerate(datasets_config.items(), 1):
            ...
            if config['type'] == 'text_to_image':
                ...
                # 立即保存单个数据集的结果
                dataset_result_file = os.path.join(results_dir, f'JinaCLIP-v2_{dataset_name}_results.json')
        ...
        # 保存结果
        model_result_file = os.path.join(results_dir, 'JinaCLIP-v2_long_eval_results.json')
        ...
        print_progress(f"JinaCLIP-v2 长文本模型评测失败: {str(e)}", "")
        print(traceback.format_exc())
        return {"JinaCLIP-v2": {'error': str(e)}}

def main():
    print_progress("开始JinaCLIP-v2长文本模型召回能力评估", "")
    
    # 配置路径
    datasets_dir = "/root/autodl-tmp/bge_vl/datasets/long_eval/datasets"
    # 将结果保存到 single_jina_eval/long_results
    results_dir = "/root/autodl-tmp/bge_vl/datasets/single_jina_eval/long_results"
    
    os.makedirs(results_dir, exist_ok=True)
    
    batch_size = 512  # 保持与原框架一致
    ...
    # 评估JinaCLIP模型
    results = evaluate_jinaclip_long_model(datasets_config, batch_size, results_dir)
    ...
    print_progress("JinaCLIP-v2长文本评测已完成!", "")
