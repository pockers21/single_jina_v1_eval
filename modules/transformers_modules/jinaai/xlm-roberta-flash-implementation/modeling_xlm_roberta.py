...
def create_mixer_cls(config, cross_attn=False, return_residual=False):
    ...
    rotary_kwargs = {}
    if config.position_embedding_type == "rotary":### here
    ...
    mixer_cls = partial(
        ...
        use_alibi=config.position_embedding_type == "alibi",# rotary
        **rotary_kwargs,
    )
    return mixer_cls
...
def create_mlp_cls(config, layer_idx=None, return_residual=False):
    ...
    # if fused_mlp: # not here
    #     assert config.hidden_act in ["gelu_new", "gelu_fast", "gelu_pytorch_tanh"], (
    #         "fused_mlp only " "supports approximate gelu"
    #     )
    if not fused_mlp:## here
        approximate = (
            "tanh"
...
def create_block(config, layer_idx=None):
    ...
    mixer_cls = create_mixer_cls(config, cross_attn, return_residual=return_residual)# false true
    mlp_cls = create_mlp_cls(config, layer_idx, return_residual=return_residual)# true
...
class XLMRobertaEncoder(nn.Module):
    ...
    def forward(
        ...
        if key_padding_mask is None or not self.use_flash_attn: #here
            mixer_kwargs = {"adapter_mask": adapter_mask}
            if key_padding_mask is not None:#here
                mixer_kwargs["key_padding_mask"] = key_padding_mask.bool()
            for layer in self.layers:
                # if output_hidden_states:#not here
                #     all_hidden_states = all_hidden_states + (hidden_states,)
                # if self._grad_checkpointing:# not here
                ...
                #else:#here
                if 1:
                    hidden_states = layer(hidden_states, mixer_kwargs=mixer_kwargs) ## here
            # if output_hidden_states:# not here
            #     all_hidden_states = all_hidden_states + (hidden_states,)
            # if subset_mask is not None:# not here
...
class XLMRobertaModel(XLMRobertaPreTrainedModel):
    def __init__(self, config: XLMRobertaFlashConfig, add_pooling_layer=True):
        ...
        if config.vocab_size % self.pad_vocab_size_multiple != 0:# not here
        ...
        self.emb_drop = nn.Dropout(config.hidden_dropout_prob)# zero并且scaled非零元素
       ...
       # self.pooler = XLMRobertaPooler(config) if add_pooling_layer else None
        ...
        print('done init robert')

        ####
       # self.emb_ln=util_weightDirect.emb_ln_getW(self.emb_ln)
       # self.encoder.layers[0].norm1=util_weightDirect.ln_norm1_getW(self.encoder.layers[0].norm1)
       # self.encoder.layers[0].norm2 = util_weightDirect.ln_norm2_getW(self.encoder.layers[0].norm2)
        print('done load weight')
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        adapter_mask = kwargs.pop("adapter_mask", None) #[0] taskid
        if kwargs: #not here
        ...
        )# 包括word_emb,token_type_id emb
        print('emb',hidden_states[0,0,:5],torch.mean(hidden_states),LA.matrix_norm(hidden_states))###[0.0151,  0.0441, -0.1420, -0.0960, -0.0285]
        # TD [2022-12:18]: Don't need to force residual in fp32
        # BERT puts embedding LayerNorm before embedding dropout.
        if not self.fused_dropout_add_ln: #here [1 8 1024]
            hidden_states = self.emb_ln(hidden_states);print('emb_ln',hidden_states[0,0,:5],LA.matrix_norm(hidden_states))#[-0.1205, -0.0683, -0.3614, -0.2110,  0.4517]
        # else: #not here
        #     hidden_states = layer_norm_fn(
        #         hidden_states, self.emb_ln.weight, self.emb_ln.bias, eps=self.emb_ln.eps
        #     )
        hidden_states = self.emb_drop(hidden_states);print('emb_drop',hidden_states[0,0,:5],LA.matrix_norm(hidden_states))# [-0.1205, -0.0683, -0.3614, -0.2110,  0.4517]

        # if masked_tokens_mask is not None: #not here
        ...
        sequence_output = self.encoder(
            ...
            subset_mask=subset_mask, # none
            adapter_mask=adapter_mask, #[0] taskid
        ...
        if masked_tokens_mask is None:# here
            pooled_output=None
            # pooled_output = (
