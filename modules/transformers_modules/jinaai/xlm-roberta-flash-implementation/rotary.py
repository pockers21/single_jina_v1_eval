...
### 这个代码就是flash-attn https://github.com/Dao-AILab/flash-attention/blob/main/flash_attn/layers/rotary.py
...
class ApplyRotaryEmbQKV_(torch.autograd.Function):
    ...
    def forward(
        ...
        if cos_k is None and sin_k is None and qkv.is_contiguous():### here

            # if use_flash_attn:# not here
        ...
        if isinstance(seqlen_offsets, int):## here
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def forward(
        ...
        # if max_seqlen is not None:### not here
        #     self._update_cos_sin_cache(max_seqlen, device=qkv.device, dtype=qkv.dtype)
        #elif isinstance(seqlen_offset, int):# here
        if 1:
            self._update_cos_sin_cache(
